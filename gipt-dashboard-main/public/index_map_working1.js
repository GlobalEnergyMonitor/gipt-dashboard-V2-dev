...
function implentGraph(id) {
    ...
    if (id === "23632161") {
        ...
        // Get currently selected country from dropdown
        const selected = getSelectedText(); // e.g. "Germany"
        const filtered = fullData.filter(entry => entry["Country/area"] === selected);
...
if (id === "23632161") {
    const fullData = config.datasets[id];
    const selected = getUnformattedInputName(key); // Correct value from dropdown
    const filtered = fullData.filter(entry => entry["Country/area"] === selected);

    // ✅ Debug: check what we're matching
    console.log("Dropdown key:", key);
    console.log("Unformatted selection for map:", selected);
    console.log("Filtered map data rows:", filtered.length);

    const containerId = `chart-${id}`;
    const container = document.querySelector(`#${containerId}`);
    container.innerHTML = ""; // remove old chart iframe
